<?php
/**
 * LearnDash Discord Service Implementation
 *
 * Handles Discord integration for LearnDash plugin.
 *
...
/**
 * Service integration for LearnDash + Discord.
 *
 * @since 1.0.0
 */
class Dro_CCBB_LearnDash extends Abstract_Service implements Service_Interface {
	...
	/**
	 * Main plugin file path used to detect whether the LearnDash Discord add-on is active.
	 *
	 * @since 1.0.0
	 * @var string
	 */
	private const PLUGIN_NAME = 'connect-learndash-and-discord/learndash-discord.php';
	 ...
	 * @var string
	 */
	private const SERVICE_NAME = 'learndash-discord-service';
	...
	/**
	 * Mapping of logical Discord meta keys to their LearnDash add-on user_meta keys.
	 *
	 * @since 1.0.0
	 * @var array<string,string>
	 */
	protected array $discord_meta_keys = array(
		'discord_username'    => '_ets_learndash_discord_username',
		'discord_user_avatar' => '_ets_learndash_discord_avatar',
		'discord_user_id'     => '_ets_learndash_discord_user_id',
		'access_token'        => '_ets_learndash_discord_access_token',
	);
	...
	/**
	 * Determine whether the LearnDash Discord add-on is active.
	 *
	 * @since 1.0.0
	 ...
	 *
	 * LearnDash does not define membership levels; return null by default.
	 *
	 * @since 1.0.0
	 * @param int $user_id The user ID.
	...
	public function get_user_access_context( int $user_id ): ?array {
		// LearnDash doesn't use membership levels like PMPro.
	...
	public function build_html_block( array $attributes, string $content, \WP_Block $block ): string {
		...
		$html = '';
		if ( function_exists( 'learndash_discord_check_saved_settings_status()' ) ) {
			if ( learndash_discord_check_saved_settings_status() && $access_token ) {
	...
	private function get_connect_button( string $button_bg_color, string $button_text_color, string $button_text ): string {
		...
		return sprintf(
			'<a href="?action=learndash-discord-login" class="dro-ccbb-connect-button" style="background-color:%s; color:%s;">%s <i>%s</i></a>',
	...
	private function get_disconnect_button( int $user_id, string $button_bg_color, string $button_text_color, string $button_text ): string {
		wp_enqueue_script( 'connect-learndash-and-discord' );
		wp_enqueue_style( 'connect-learndash-and-discord' );

		$button_html = sprintf(
			'<a href="#" class="dro-ccbb-disconnect-button" id="learndash-discord-disconnect-discord" data-user-id="%s" style="background-color:%s; color:%s;">%s <i>%s</i></a>',
			esc_attr( $user_id ),
			esc_attr( $button_bg_color ),
			esc_attr( $button_text_color ),
			esc_html( $button_text ),
